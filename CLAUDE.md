...
- value types: nil, bool, int (i64), float (f64), string (*ObjString), function (*ObjFunction), struct_ (*ObjStruct), enum_ (*ObjEnum), native_fn (*ObjNativeFn), closure (*ObjClosure), array (*ObjArray), task (*ObjTask), channel (*ObjChannel), listener (*ObjListener), conn (*ObjConn), dgram (*ObjDgram), ptr (raw C pointer for FFI)
...
- 204 tests, 26 validated examples, 10 benchmarks
...
**next:** std/tls, package manager / module resolution, error handling (?T option types, ?? operator)
